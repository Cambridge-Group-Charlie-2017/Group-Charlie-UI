...
import { RemoteMessage, Content } from '../../services/api';
    ...
    render() {
        ...
        return <div className="EmailView top">
            ...
            {this.state.content ? <div className="EmailView attContainer">{
                this.state.content.attachment.map(name => <div className="EmailView attachment" title={name} onClick={() => this.openAttachment(name)} key={name}><Icon name="file-o" />{name}</div>)}
            </div> : null}
